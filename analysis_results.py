...
try:
    mom_an = np.load('./analytical_mom/analytical_mom_nside%s_fsky%s_scale%s_Nlbin%s_d%ss%s_%s_%s%s%s_%s%s.npy' % (nside, fsky, scale, Nlbin, dusttype, synctype, mode, np.round(betabar,3), np.round(tempbar,3), np.round(betasbar,3),int(nu0d),int(nu0s)), allow_pickle=True).item()
except:
    ...
    np.save('./analytical_mom/analytical_mom_nside%s_fsky%s_scale%s_Nlbin%s_d%ss%s_%s_%s%s%s_%s%s.npy' % (nside, fsky, scale, Nlbin, dusttype, synctype, mode, np.round(betabar,3), np.round(tempbar,3), np.round(betasbar,3),int(nu0d),int(nu0s)), mom_an)
