...
Nell = len(leff)
Pathload = '/global/cfs/cdirs/litebird/results/b_modes_postptep/gnilc_maps_v1/%s/output_total' % (complexity) # Path to the GNILC maps on the NERSC
...
Bls = hp.gauss_beam(beam, lmax=3*nside-1, pol=True).T[2] * np.ones((Nfreqs, 3*nside))
...
if load:
    if masking_strat == '':
        DLgnilc = np.load(path+'power_spectra/DLgnilc_nside%s_fsky%s_scale%s_Nlbin%s_d%ss%sc_gaussbeam_bandpass.npy' % (nside, fsky, scale, Nlbin, fg_type, fg_type))
    else:
        DLgnilc = np.load(path+'power_spectra/DLgnilc_nside%s_%s_scale%s_Nlbin%s_d%ss%sc_gaussbeam_bandpass.npy' % (nside, masking_strat, scale, Nlbin, fg_type, fg_type))
...
for k in trange(k_ini, N):
    ...
    for i in range(Nfreqs):
        maps[i] = hp.read_map(Pathload+'/%05d/QU_output_total_LB_%s_70.5acm_ns64_lmax191_%05d.fits' % (k, bands[i], k))
    ...
    # Save
    if masking_strat == '':
        np.save(path+'power_spectra/DLgnilc_nside%s_fsky%s_scale%s_Nlbin%s_d%ss%sc_gaussbeam_bandpass.npy' % (nside, fsky, scale, Nlbin, fg_type, fg_type), DLgnilc)
    else:
        np.save(path+'power_spectra/DLgnilc_nside%s_%s_scale%s_Nlbin%s_d%ss%sc_gaussbeam_bandpass.npy' % (nside, masking_strat, scale, Nlbin, fg_type, fg_type), DLgnilc)